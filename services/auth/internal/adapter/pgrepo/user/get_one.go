...
func (us *userRepository) UserByVerifyToken(
	ctx context.Context,
	verifyToken string,
) (*entity.User, error) {
	const op = "adapter.pgrepo.user.UserByResetToken"

	const query = "SELECT * FROM %s WHERE verify_token=$1"
	...
	var user entity.User
	err = stmt.GetContext(ctx, &user, token)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, adapter.ErrUserNotFound
		}
...
func (us *userRepository) UserByResetToken(
	ctx context.Context,
	resetToken string,
) (*entity.User, error) {
	const op = "adapter.pgrepo.user.UserByResetToken"

	const query = "SELECT * FROM %s WHERE reset_token=$1"
